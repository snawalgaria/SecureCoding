...
\textbf{Impact} & Medium/High, depending on the victim's privileges. By stealing the session from a user (admin or not), we would be able to impersonate them and do actions in their name. \\
\textbf{Likelihood} & High \\
...
\textbf{Impact} & Medium/High, depending on the victim's privileges. An attacker might create a session and make the user authenticate with it, thus giving the attacker access to the victim's session. \\
...
\begin{tabular}{l|p{10cm}}
\textbf{Observation} & The cookies are sent over an unencrypted connection. Cookies also aren't renewed when logging in. \\
\textbf{Discovery} & BurpSuite \\
\textbf{Impact} & Medium/High, depending on the victim's privileges. By using MitM to steal the session from a user (admin or not), we would be able to impersonate them and do actions in their name. This compromises the integrity of the application. \\
\textbf{Likelihood} & High \\
\textbf{Access Vector} & Adjacent Network \\
\textbf{Access Complexity} & Low\\
\textbf{Privileges Required} & None \\
...
\begin{tabular}{l|p{10cm}}
\textbf{Observation} & There is no mechanism to prohibit CSRF. GET requests aren't enough, though, since all of the important data is only transmitted via POST requests. \\
\textbf{Discovery} & Manual test \\
\textbf{Impact} & Medium/High, depending on the victim's privileges. By getting our victim to open a malicious web page, that creates a POST request with JS, we would be able to perform actions in their name. \\
\textbf{Likelihood} & High \\
\textbf{Access Vector} & Network \\
\textbf{Access Complexity} & Low\\
\textbf{Privileges Required} & None \\
...
\textbf{Discovery} & Manual test \\
\textbf{Impact} & Medium/High, depending on the victim's privileges. All people using the same (maybe public) machine afterwards have access to the victim's account. \\
\textbf{Likelihood} & High \\
...
\begin{tabular}{l|p{10cm}}
\textbf{Observation} & When trying all HTTP methods, CONNECT returns "Bad Request", while OPTIONS returns "OK". Otherwise, all methods except GET, HEAD and POST are not allowed\\
\textbf{Discovery} & script http\_tampering.sh in phase2/scripts/ (from OWASP website) \\
\textbf{Impact} & The web server might respond in unexpected ways to these methods and might execute scripts without privilege control when using HEAD instead of GET. \\
\textbf{Likelihood} & n/a \\
\textbf{CVSS} & n/a \\
\end{tabular}
...
\textbf{Observation} & When uploading transaction files, it's possible to enter negative amounts, which results in gaining money at the "recipient's" cost. \\
\textbf{Discovery} & Manual testing \\
\textbf{Impact} & Attackers can steal money from anybody they want \\
\textbf{Likelihood} & High \\
\textbf{Access Vector} & Network \\
\textbf{Access Complexity} & Low \\
\textbf{Privileges Required} & Low \\
...
\textbf{Discovery} & Browsing the app. \\
\textbf{Impact} & n/a \\
\textbf{Likelihood} & n/a \\
\textbf{CVSS} & n/a \\
\end{tabular}
...
\textbf{Discovery} & ZAP, W3AF \\
\textbf{Impact} & This itself is barely a problem, since at best it DDOSes the app. But that would be noticeable then. It makes it easier for an attacker to find other vulnerabilities, though. \\
\textbf{Likelihood} & High \\
\textbf{Access Vector} & Network \\
\textbf{Access Complexity} & Low \\
\textbf{Privileges Required} & None \\
